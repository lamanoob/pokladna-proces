...
function objednavkaNaUcet($spojeni, $uzivatel, $ouid){
  ...
  $mnoz = post('mnozstvi');
  $pol = post('polovicni');
  $recpovolene = $obj = $rec = $kVytisteni = $prikazPole = $kVytisteni = array();
  ...
  foreach($obj AS $data){
    ...
    if((is_numeric($key))&&($key > 0)){
      ...
      /*****/
      $kVytisteni[$tiskarnaRec][] = array($mnozstvi."x", $nazev, $cena); 
      }
    }
  ...
  if((!_tiskPovolen_) && (!empty($kVytisteni))){
    ...
    foreach($kVytisteni AS $tiskarna => $textPole){
      ...
      $tisk->fill("=");
      $tisk->wrap();
      $tisk->insert("OBJEDNAVKA");
      $tisk->wrap();
      $tisk->fill("=");
    
      
    
      $velikosti = array(5, 0, 10);
    ...
    foreach($kVytisteni AS $tiskarna => $text){
      ...
      if(isset($tiskarny[$tiskarna])){
        ...
        $tisk->write($tiskNadpis.$text, 'utf2win');
